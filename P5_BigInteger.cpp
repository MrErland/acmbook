...
struct BigInteger {
	...
	BigInteger operator - (const BigInteger& b) const {       // 减法 注意减数和被减数可能交换位置
		...
		if (*this > b)
		{
			...
			if (c.s.back() == 0) c.s.pop_back();			// 最高位存在0，则删除
		}
		else         // 如果减数大于被减数，互换位置，并在结果的高位加上负号
		{
			for (unsigned int i = 0, g = 0;; i++) {
				if (g == 0 && i >= s.size() && i >= b.s.size()) break;
				int x = g;
				if (i < b.s.size()) x = b.s[i] - g;        // 修改
				if (i < s.size()) x -= s[i];           // 修改
				if (x >= 0) { g = 0; c.s.push_back(x % BASE); }
				else { g = 1; c.s.push_back((x + BASE) % BASE); }
			}
			if (c.s.back() == 0 && c.s.size() > 1) c.s.pop_back();		// 最高位存在0，则删除
	...
	bool operator < (const BigInteger& b) const{             // 小于  0.3-增加判断负数
		int mark = 0;
		if (s.empty() || b.s.empty()) return false;
		mark = s[s.size() - 1] * b.s[b.s.size() - 1];		// 计算最高位的符号
		if (mark == 0)				// 存在0
		{
		...
		else if (mark > 0)			// 同号
		{
			if (s.size() != b.s.size())
			{
				if(s[s.size() - 1] > 0) return s.size() < b.s.size();
				else return b.s.size() < s.size();
			}
			for (int i = s.size() - 1; i >= 0; i--)
				if (s[i] != b.s[i])
				{
					if(s[s.size() - 1] > 0) return s[i] < b.s[i];
					else return b.s[i] < s[i];
				}
			return false;      // 相等
		}
		else					// 异号
